...
class _MyHomePageState extends State<MyHomePage> {
  List<String> listUrl = ["https://producthabits.com/stop-ignoring-competitors/",
            ...
            child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  return FlutterLinkPreview(
                    ...
                    builder: (info) {
                      if (info is WebInfo) {
                        String domain = listUrl[index].split(".")[listUrl[index].split(".").length - 2].split("://")[1];

                        return Container(
                          margin: EdgeInsets.all(20),
                          padding: EdgeInsets.all(10),
                          decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), boxShadow: [BoxShadow(spreadRadius: 1, blurRadius: 5, color: Colors.grey)]),
                          height: MediaQuery.of(context).size.height * 0.45,
                          width: MediaQuery.of(context).size.width * 0.85,
                          child: Column(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              Container(child: Text(info.title, maxLines: 3, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold))),
                              SizedBox(height: 10),
                              Container(
                                height: 200,
                                child: info.image != null
                                    ...
                                    : FlutterLinkPreview(
                                        url: "https://" + listUrl[index].split("/")[2],
                                        builder: (info) {
                                          return info is WebInfo && info.image != null ? Image.network(info.image) : Container();
                                        },
                                      ),
                              ),
                              SizedBox(
                              ...
                              Align(
                                ...
                                child: Row(
                                  ...
                                  children: [Icon(Icons.thumb_up), Text(domain, style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold))],
                                ),
                              )
                            ],
                          ),
